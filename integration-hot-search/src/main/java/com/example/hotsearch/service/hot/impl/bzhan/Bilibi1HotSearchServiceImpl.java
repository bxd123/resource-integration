package com.example.hotsearch.service.hot.impl.bzhan;
...
@Slf4j
@Service
public class Bilibi1HotSearchServiceImpl extends HttpClient implements HotSearchService {

    public Bilibi1HotSearchServiceImpl(@Autowired Map<String, String> baseUrlMap) {
        super(baseUrlMap.get(UrlConstant.APIS_JXCXIN));
    }
    ...
    @Override
    public R searchHot() {
        final WebTarget webTarget = this.webTarget.path("api/bilihot");
        try {
            Response response = asyncGetRequest(webTarget).get();
            String result = response.readEntity(String.class);
//            log.info("获取bilibil热搜数据 --》》》》  " + result);
