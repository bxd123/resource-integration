...
@Service
@Slf4j
public class TouTiaoSearchHotImpl extends HttpClient implements TouTiaoService {

    public TouTiaoSearchHotImpl(@Autowired Map<String, String> baseUrlMap) {
    ...
    @Override
    public R searchHot() {
        final WebTarget webTarget = this.webTarget.path("v2/toutiaohot");
        try {
     ...
     */
    @Override
    public R searchHotNew() {

        final WebTarget webTarget = this.webTarget.path("v2/toutiaohotnew");
        try {
            Response response = asyncGetRequest(webTarget).get();
            String result = response.readEntity(String.class);
            val json = JSONUtil.toBean(result, HotData.class);
